...
struct InventorySlot {
    ...
    int count = 0;
    // Оставшаяся прочность, если предмет попал в этот слот СНЯТЫМ с себя (см. unequip()) — 0 у предметов без
    // durability-механики и у ещё ни разу не надёванных (тогда при экипировке берётся полная ItemDefinition::durability,
    // см. useBagSlot()). Не участвует в стаковке (maxStack==1 у всей брони, см. ItemDefinition.cpp) — задваивать
    // прочность разных экземпляров было бы некорректно, но у нас такой ситуации и не возникает.
    int durability = 0;
    bool isEmpty() const
    {
        return item == nullptr;
...
public:
    ...
    // считать предмет подобранным. durability — только для unequip() ниже (снятая с себя броня уходит в мешок с
    // тем, что от неё осталось, а не как новая); обычный подбор с карты/сундука его не передаёт (0 по умолчанию —
    // "предмет ещё не надёван", при экипировке возьмётся полная ItemDefinition::durability).
    bool addItem(const ItemDefinition& item, int count = 1, int durability = 0);
    ...
    // Снимает экипировку категории обратно в мешок, если там есть место. Оставшаяся прочность (см.
    // ItemDefinition::durability) едет в мешок вместе с предметом (см. InventorySlot::durability) — надев его
    // обратно, получаем ровно то, что от него осталось, а не полный запас заново (был баг — так чинили сломанную
    // броню бесплатным снятием/надеванием).
