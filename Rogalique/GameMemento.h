...
public:
    ...
    // выбрано для этого запуска процесса, вместо мусора. Форма самой планировки (комнаты/коридоры) больше не
    // сидится вовсе — она фиксирована, см. buildBigLevelLayout — сидится только то, какой бот/предмет в каком
    // из заготовленных слотов оказался.
    ...
    // арены, как и раньше. См. arenaWave < 0 внутри самого поля — "сохранился на арене, но между волнами" не
    // бывает (currentWaveCleared() сразу запускает следующую в тот же кадр), так что >=0 однозначно значит "был в
    // разгаре боя".
