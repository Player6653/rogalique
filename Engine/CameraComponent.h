...
// Камера — часть окна, центр которой автоматически следует за позицией владельца. apply() вызывается явно из
// Engine::run() один раз за кадр, до отрисовки сцены — не часть обычного прохода GameObject::draw(), иначе то,
// что применяет вид, зависело бы от порядка отрисовки объектов (и рисковало бы выполниться уже после того, как
// что-то нарисовалось поверх).
